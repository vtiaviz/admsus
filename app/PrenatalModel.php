...
{
    public static function listGestantes($ubs, $equipe, $microarea)
    {
        ...
        FROM 
            ...
            FROM 
                ...
                RIGHT JOIN 
                    ...
                    FROM 
                        ...
                        AND CASE WHEN $ubs <> 0 THEN tfcp.co_dim_unidade_saude_vinc = $ubs ELSE TRUE END
        
                        AND CASE WHEN $equipe <> 0 THEN tde.co_seq_dim_equipe = $equipe ELSE TRUE END
        
                        AND CASE 
        
                                WHEN $microarea = '0' THEN TRUE 
        
                                WHEN $microarea = 'CS' THEN TRUE
                    ...
                    FULL OUTER JOIN
                        ...
                        AND CASE WHEN $ubs <> 0 THEN tfci.co_dim_unidade_saude = $ubs ELSE TRUE END
        
                        AND CASE WHEN $equipe <> 0 THEN tfci.co_dim_equipe = $equipe ELSE TRUE END
                        ...
                        AND CASE 
                                ...
                                ELSE tfci.nu_micro_area = $microarea 
            ...
            FULL JOIN 
                ...
                AND CASE WHEN $ubs <> 0 THEN cad.co_dim_unidade_saude = $ubs ELSE TRUE END
        
                AND CASE WHEN $equipe <> 0 THEN cad.co_dim_equipe = $equipe ELSE TRUE END
                ...
                AND CASE 
                        ...
                        ELSE cad.nu_micro_area = $microarea 
        ...
        AND CASE WHEN $microarea <> 'CS' THEN TRUE ELSE gestante.micro = 'CS' END 
        ...
        ORDER BY gestante.no_cidadao");

        return $listaGestantes;

    }

    public static function getGestante($cidadao)
    {
        
        $getGestante = \DB::select("SELECT DISTINCT
        ...
        gestante.atualizacao,
    
        gestante.micro,
    
        CASE WHEN (date_trunc('DAY', CURRENT_DATE) - INTERVAL '50 WEEK') < gestante.dum THEN gestante.dum END dum,
        ...
        (date_trunc('DAY', gestante.dum) + INTERVAL '42 WEEK') AS ini_puerperio,
    
        CASE WHEN desfecho.puerpera_1_consul IS NOT NULL THEN 1 ELSE 0 END AS puerpera,
    
        MAX(desfecho.puerpera_1_consul) puerpera_1_consul,
    
        gestante.semanas,
    
        gestante.semana_consulta_1,
    
        vdrl.vdrl_solicitacao,
        ...
        MAX(odonto.dt_consulta) AS ultimo_odonto,
    
        CASE WHEN MAX(odonto.dt_consulta) IS NOT NULL THEN 1 END consul_odonto,
    
        CASE WHEN dtpa.data_vacina >= gestante.dum THEN dtpa.data_vacina ELSE NULL END AS dtpa
        ...
        FROM 
            ...
            AND tfai.co_fat_cidadao_pec = $cidadao 
            ...
            GROUP BY 
                ...
                RIGHT JOIN 
                    ...
                    FROM 
                        ...
                        AND tfcp.co_seq_fat_cidadao_pec = $cidadao
            ...
            FULL JOIN 
                ...
                AND cad.co_dim_tempo = tdt2.co_seq_dim_tempo 
        
                AND cad.co_fat_cidadao_pec = tfcp2.co_seq_fat_cidadao_pec 
        
                AND tfcp2.co_seq_fat_cidadao_pec = $cidadao
            ...
            ON clinico.cidadao = referido.cidadao
        ...
        LEFT JOIN 
            ...
            WHERE co_fat_cidadao_pec = $cidadao 
        ...
        LEFT JOIN 
            ...
            (SELECT DISTINCT
                ...
                CASE 
                    ...
                    WHEN exame.cidadao IS NOT NULL THEN exame.cidadao
        
                    WHEN vdrl_r.cidadao IS NOT NULL THEN vdrl_r.cidadao
        
                END AS cidadao,
        
                CASE WHEN exame.vdrl_solicitacao IS NOT NULL THEN 1 END vdrl_solicitacao,
        
                CASE WHEN exame.vdrl_avaliacao IS NOT NULL THEN 1 END vdrl_avaliacao,
            ...
            FROM 
                ...
                SELECT DISTINCT 
                    ...
                    CASE 
                        ...
                        WHEN vdrl_s.cidadao IS NOT NULL THEN vdrl_s.cidadao
        
                        WHEN vdrl_a.cidadao IS NOT NULL THEN vdrl_a.cidadao
        
                    END AS cidadao,
        
                    CASE WHEN vdrl_s.solicitacao_vdrl IS NOT NULL THEN 1 END vdrl_solicitacao,
                ...
                FROM
                    ...
                    (SELECT DISTINCT
                        ...
                        MAX(tfai5.dt_inicial_atendimento) solicitacao_vdrl
                    ...
                    AND tfai5.co_fat_cidadao_pec = $cidadao 
        
                    AND tfai5.co_dim_sexo = 2
        
                    AND (tfai5.ds_filtro_proced_solicitados LIKE '%ABEX019%'
        
                        OR tfai5.ds_filtro_proced_solicitados LIKE '%0202031110%'
        
                        OR tfai5.ds_filtro_proced_solicitados LIKE '%0202031179%')
        
                    GROUP BY tfai5.co_fat_cidadao_pec) AS vdrl_s
                ...
                ON vdrl_s.cidadao = vdrl_a.cidadao
            ...
            FULL JOIN
                ...
                AND tfpap.co_fat_cidadao_pec = $cidadao 
            ...
            ON exame.cidadao = vdrl_r.cidadao) vdrl
        ...
        LEFT JOIN
            ...
            (SELECT DISTINCT
                ...
                CASE WHEN exame.hiv_solicitacao IS NOT NULL THEN 1 END hiv_solicitacao,
        
                CASE WHEN exame.hiv_avaliacao IS NOT NULL THEN 1 END hiv_avaliacao,
            ...
            FROM 
                ...
                SELECT DISTINCT 
                    ...
                    CASE 
                        ...
                        WHEN hiv_s.cidadao IS NOT NULL THEN hiv_s.cidadao
        
                        WHEN hiv_a.cidadao IS NOT NULL THEN hiv_a.cidadao
        
                    END AS cidadao,
        
                    CASE WHEN hiv_s.solicitacao_hiv IS NOT NULL THEN 1 END hiv_solicitacao,
                ...
                FROM
                    ...
                    (SELECT DISTINCT
                        ...
                        MAX(tfai6.dt_inicial_atendimento) solicitacao_hiv
                    ...
                    AND tfai6.co_fat_cidadao_pec = $cidadao 
        
                    AND tfai6.co_dim_sexo = 2
        
                    AND (ds_filtro_proced_solicitados LIKE '%ABEX018%'
        
                        OR ds_filtro_proced_solicitados LIKE '%0202030300%')
        
                    GROUP BY tfai6.co_fat_cidadao_pec) hiv_s
                ...
                ON hiv_s.cidadao = hiv_a.cidadao
            ...
            FULL JOIN
                ...
                WHERE tfpap2.co_dim_tempo >= (
                    ...
                    AND tfpap2.co_fat_cidadao_pec = $cidadao 
            ...
            ON exame.cidadao = hiv_r.cidadao) hiv
        ...
        LEFT JOIN
            ...
            SELECT
                ...
                WHEN reg.cidadao IS NULL THEN cds.cidadao
        
                WHEN reg.cidadao IS NOT NULL THEN reg.cidadao
        
                WHEN cds.cidadao IS NULL THEN reg.cidadao
        
                WHEN cds.cidadao IS NOT NULL THEN cds.cidadao
        
                END cidadao,
        
                CASE 
        
                WHEN reg.data_vacina IS NULL THEN cds.data_vacina
        
                WHEN reg.data_vacina IS NOT NULL AND reg.data_vacina >= cds.data_vacina THEN reg.data_vacina
        
                WHEN cds.data_vacina IS NULL THEN reg.data_vacina
        
                WHEN cds.data_vacina IS NOT NULL AND cds.data_vacina > reg.data_vacina THEN cds.data_vacina
            ...
            FROM
                ...
                SELECT DISTINCT
                    ...
                    cid.co_seq_fat_cidadao_pec cidadao,
        
                    vac.dt_aplicacao data_vacina,
        
                    vac.co_dose_imunobiologico dose
        
                FROM
        
                    tb_fat_cidadao_pec cid,
                    ...
                    tb_registro_vacinacao vac,
        
                    tb_dose_imunobiologico dose,
        
                    tb_imunobiologico
        
                WHERE   cid.co_cidadao = tb_cidadao.co_seq_cidadao
        
                AND cid.co_seq_fat_cidadao_pec = $cidadao
        
                AND dose.co_dose_imunobiologico <> 0
        
                AND tb_imunobiologico.sg_imunobiologico = 'dTpa adulto'
        
                AND tb_vacinacao.co_seq_vacinacao = vac.co_vacinacao
        
                AND tb_cidadao.co_seq_cidadao = tb_prontuario.co_cidadao
        
                AND tb_prontuario.co_seq_prontuario = tb_vacinacao.co_prontuario
        
                AND dose.co_dose_imunobiologico = vac.co_dose_imunobiologico
        
                AND vac.co_imunobiologico = tb_imunobiologico.co_imunobiologico
        
                AND vac.co_tipo_registro_vacinacao = 1
        
                AND vac.st_registro_anterior = 1) AS reg
            ...
            FULL JOIN 
                ...
                SELECT DISTINCT
                    ...
                    gest.co_fat_cidadao_pec cidadao,
        
                    dat.dt_registro data_vacina,
        
                    vac.co_dim_dose_imunobiologico dose
        
                FROM
        
                    tb_fat_vacinacao_vacina vac,
        
                    tb_fat_vacinacao gest,
        
                    tb_dim_tempo dat,
        
                    tb_dim_imunobiologico imuno
        
                WHERE   vac.co_fat_vacinacao = gest.co_seq_fat_vacinacao
        
                AND gest.co_fat_cidadao_pec = $cidadao
        
                AND gest.co_fat_cidadao_pec IS NOT NULL
        
                AND vac.co_dim_imunobiologico = imuno.co_seq_dim_imunobiologico
        
                AND imuno.sg_imunobiologico = 'dTpa adulto'
        
                AND vac.co_dim_dose_imunobiologico <> 0
        
                AND vac.co_dim_tempo = dat.co_seq_dim_tempo
        
                ) cds
        
            ON reg.cidadao = cds.cidadao
        
        ORDER BY cidadao) dtpa
        
        ON dtpa.cidadao = gestante.cidadao
        ...
        LEFT JOIN 
            ...
            SELECT DISTINCT 
                ...
                MAX(atend.dt_inicial_atendimento) puerpera_1_consul
            ...
            AND atend.co_fat_cidadao_pec = $cidadao
